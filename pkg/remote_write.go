...
func NewClient(remoteWriteURL string, userAgent string, writeFrequency string) (RemoteWriteClient, error) {
	...
	err := client.cron.AddFunc(writeFrequency, client.pushMetrics)
	if err != nil {
...
func (client *RemoteWriteClient) pushMetrics() {
	...
	if res.StatusCode/100 != 2 {
		return
		// stringErr, _ := io.ReadAll(res.Body)
		// return fmt.Errorf("remote write request failed with status code: %d, error: %v", res.StatusCode, string(stringErr))
	}
